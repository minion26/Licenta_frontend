...
function SeeAdminsAdmin() {
  ...
  return (
      <div>
          <Header/>
          <UpperHeader title={"See accounts"} subtitle={"Admins"} buttons={[{ key: "Search", label: "Search" }]} onSearch={handleSearch}/>
                          ...
                          <CardElongated key={index} title={`${admin.firstName} ${admin.lastName}`} cardIndex={index+1} height={145}>
                              <Box sx={{ display: "flex" }}>
                                  ...
                                  >
                                      ...
                                      onClick={() => {
                                                fetch(`http://localhost:8081/api/v1/users/make-superuser/${admin.idUsers}`, {
                                                    method:'POST',
                                                    credentials: 'include',
                                                    headers: {
                                                        'Content-Type': 'application/json',
                                                    ...
                                                    .then(response => {
                                                        if (!response.ok) {
                                                            console.error(`Server responded with status code ${response.status}`);
                                                            throw new Error('Failed to make the account an admin');
                                                        }
