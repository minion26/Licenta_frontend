...
import styles from "../See-Students-Answers-To-Questions-Per-Test-Teacher/See-Students-Answers-To-Questions-Per-Test-Teacher.module.css";
...
function SeeSubmittedTestStudent() {
  ...
  useEffect(() => {
    fetch(
      `http://localhost:8081/api/v1/student-answers/see-my-answer/idExam=${idExam}`,
      {
        method: "GET",
        credentials: "include",
        headers: {
  ...
  return (
    <div>
      ...
      <div className={styles.container}>
        {!hasError && studentsAnswer.length > 0 ? (
          studentsAnswer.map((answer, index) => {
            return (
              <Box
                key={index}
                sx={{
          ...
          <h1>You dont have the permission to see this.</h1>
        ) : (
