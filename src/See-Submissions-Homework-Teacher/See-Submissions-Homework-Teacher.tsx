...
function SeeSubmissionsHomeworkTeacher() {
  ...
  useEffect(() => {
    fetch(
      `http://localhost:8081/api/v1/homework-announcements/idHomeworkAnnouncement=${idHomeworkAnnouncement}`,
      {
        method: "GET",
        credentials: "include",
        headers: {
  ...
  useEffect(() => {
    fetch(
      `http://localhost:8081/api/v1/homework/all/idHomeworkAnnouncement=${idHomeworkAnnouncement}`,
      {
        method: "GET",
        credentials: "include",
        headers: {
  ...
  const handleSubmit =
    (idHomework: string) => (event: React.FormEvent<HTMLFormElement>) => {
  ...
  const handlePatch = async (idHomework: string) => {
    const response = await fetch(
      `http://localhost:8081/api/v1/homework/grade/idHomework=${idHomework}`,
      {
        method: "PATCH",
        credentials: "include",
        headers: {
  ...
  return (
    <div>
      ...
      <UpperHeader
        ...
        subtitle={homeworkAnn ? homeworkAnn.title : ""}
      />
      <div className={styles.container}>
        {homeworks.length === 0 ? (
          ...
          >
            <div className={styles.title}>
              <p className={styles.p}>
                No homeworks submitted by students yet.
              </p>
            </div>
          </Card>
        ) : (
          homeworks.map((homework, index) => (
            <div key={index}>
              <CardElongated
                title={`${homework.firstNameStudent} ${homework.lastNameStudent}`}
                description={
                  "Grade : " +
                  (homework
                    ? homework.grade != -1
              ...
              >
                ...
                <form onSubmit={handleSubmit(homework.idHomework)}>
                  <DialogContent>
                    <DialogContentText>
                      To give or edit a grade, please enter a value between 0
                      and 10.
                    </DialogContentText>
