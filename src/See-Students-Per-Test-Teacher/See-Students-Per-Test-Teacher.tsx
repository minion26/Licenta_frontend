...
function SeeStudentsPerTestTeacher(){
    ...
    const handleClickOpen = ({idStudentExam} : {idStudentExam : string}) => {
        ...
        fetch(`http://localhost:8081/api/v1/student-exam/get/idStudentExam=${idStudentExam}`, {
            method: "GET",
            credentials: 'include',
            headers: {
                "Content-Type": "application/json",
    ...
    const handleSubmit =  (event: React.FormEvent<HTMLFormElement> , {idStudentExam} : {idStudentExam : string}) => {
    ...
    const handlePATCH = async (idStudentExam: string) => {
        const response = await fetch (`http://localhost:8081/api/v1/student-exam/update/idStudentExam=${idStudentExam}`, {
            method: 'PATCH',
            credentials: 'include',
            headers: {
                'Content-Type': 'application/json',
    ...
    return (
        <div>
            <UpperHeader title={exam.name} subtitle={new Date(exam.date).toLocaleDateString()} />
            <Header/>
                    ...
                    studentExam.map((student, index) => (
                        <div key={index}>
                                    ...
                                    ` Score : ${student.score === -1 ? "Not graded" : student.score}, Exam Status: ${
                                        (() => {
                                            switch(student.examStatus) {
                                                ...
                                                default: return student.examStatus;
                                            }
                                        })()
                                    }`
                                }
                                    ...
                                    >
                                            ...
                                            onClick={() => {
                                                Swal.fire({
                                                    ...
                                                    if (result.isConfirmed) {
                                                        fetch(`http://localhost:8081/api/v1/student-exam/delete/idStudentExam=${student.idStudentExam}`, {
                                                            method: 'DELETE',
                                                            credentials: 'include',
                                                            headers: {
                                                                'Content-Type': 'application/json',
                                        ...
                                        <Tooltip title="Edit Test Details">
                                            <Button
                                                ...
                                                onClick={() => handleClickOpen({ idStudentExam: student.idStudentExam })}
                            ...
                            <Dialog open={open} onClose={handleClose} aria-labelledby="form-dialog-title">
                                ...
                                <form onSubmit={(event) => handleSubmit(event, {idStudentExam: studentEdit.idStudentExam})}>
                                    <DialogContent>
                                        <DialogContentText>
                                            To edit the student, please change the score or the exam status.
                                        </DialogContentText>
                                        <TextField
                                            autoFocus
                                            margin="dense"
