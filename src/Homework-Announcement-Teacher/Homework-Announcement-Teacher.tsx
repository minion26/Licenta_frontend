...
function HomeworkAnnouncementTeacher() {
  ...
  const [homeworkAnnouncement, setHomeworkAnnouncement] =
    useState<HomeworkAnnouncementsCreationDTO>({
      title: "",
      description: "",
      dueDate: null,
      score: 0,
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const response = await fetch(
      `http://localhost:8081/api/v1/homework-announcements/create/idLecture=${idLectures}`,
      {
        method: "POST",
        credentials: "include",
        headers: {
  ...
  return (
    <form onSubmit={handleSubmit}>
      <Header />
      <UpperHeader title={"Create Homework Announcement"} subtitle={"date"} />
      <Box
        sx={{
          display: "flex",
      ...
      >
        <Box>
          ...
          <TextField
            ...
            value={
              homeworkAnnouncement.dueDate
                ? homeworkAnnouncement.dueDate.toISOString().split("T")[0]
                : ""
            }
            name="dueDate"
            onChange={(e) =>
