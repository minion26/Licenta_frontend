...
function AddCorrectAnswersTestTeacher() {
  ...
  useEffect(() => {
    setQuestionsWithAnswers(
      questions.filter((question) => question.correctAnswers.length > 0),
    );
  }, [questions]);

  const [questionsWithoutAnswers, setQuestionsWithoutAnswers] = useState<
    QuestionDTO[]
  >([]);
  useEffect(() => {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const sumOfScores = questionsWithoutAnswers.reduce(
      (sum, question) => sum + question.correctAnswers[0].score,
      0,
    );

    // Check if the sum of scores is equal to the total score of the exam
    if (sumOfScores !== exam.totalScore) {
      Swal.fire({
        icon: "error",
        title: `The sum of scores is not equal to the total score of the exam : ${sumOfScores} / ${exam.totalScore}`,
        showConfirmButton: false,
        // timer: 1500
      });
      return;
    ...
    const transformedQuestionsWithoutAnswers = questionsWithoutAnswers.reduce(
      (acc, question) => {
    ...
    console.log(
      "transformedQuestionsWithoutAnswers",
      JSON.stringify(transformedQuestionsWithoutAnswers),
    );

    fetch(
      `http://localhost:8081/api/v1/correct-answers-exam/createByExam/idExam=${idExam}`,
      {
        method: "POST",
        credentials: "include",
        headers: {
      ...
      <div className={styles.container}>
        {questionsWithAnswers.map((question, index) => {
          const handleScoreChange = (
            ...
            newQuestions[index].correctAnswers[0].score = Number(
              event.target.value,
            );
          ...
          const handleAnswerChange = (
            ...
            const newQuestions = [...questionsWithAnswers];
            newQuestions[index].correctAnswers[0].correctAnswer =
              event.target.value;
            setQuestionsWithAnswers(newQuestions);
          };
          ...
          return (
            ...
            >
              ...
              <TextField
                ...
                id={`score${index}`}
                value={
                  question.correctAnswers[0] && question.correctAnswers[0].score
                    ? question.correctAnswers[0].score
                    : 0
                }
              ...
              <TextField
                ...
                value={
                  ...
                  question.correctAnswers[0].correctAnswer
                    ? question.correctAnswers[0].correctAnswer
                    : ""
                }
        ...
        >
          <Button
            variant="contained"
            onClick={() => {
              const transformedQuestionsWithAnswers =
                questionsWithAnswers.reduce(
                  ...
                  {} as Record<
                    string,
                    { correctAnswer: string; score: number }
                  >,
                );

              console.log(
                "transformedQuestionsWithAnswers",
                JSON.stringify(transformedQuestionsWithAnswers),
              );
              ...
              if (Object.keys(transformedQuestionsWithAnswers).length === 0) {
                ...
                fetch(
                  `http://localhost:8081/api/v1/correct-answers-exam/updateByExam/idExam=${idExam}`,
                  {
                    method: "PATCH",
                    credentials: "include",
                    headers: {
