...
function Buttons({
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await fetch(
        `http://localhost:8081/api/v1/lectures/${idLectures}`,
        {
          method: "GET",
          credentials: "include",
          headers: {
  ...
  const handleDeleteWeek = () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`http://localhost:8081/api/v1/lectures/delete/${idLectures}`, {
          method: "DELETE",
          credentials: "include",
          headers: {
            "Content-Type": "application/json",
  ...
  const handlePatch = async () => {
    const response = await fetch(
      `http://localhost:8081/api/v1/lectures/update/${idLectures}`,
      {
        method: "PATCH",
        credentials: "include",
        headers: {
    ...
    <div>
        ...
        >
          ...
          >
            <Button
              ...
              component={Link}
              to={`/add-materials-per-lecture/${idCourses}/${idLectures}`}
            >
              {" "}
              Add Materials
            </Button>
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          ...
          <DialogActions>
            <Button onClick={handleDeleteWeek} color="primary">
              Delete this lecture
            </Button>
...
function MaterialsPerCourseTeacher() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(
          `http://localhost:8081/api/v1/materials/get-type/idLecture=${idLectures}`,
          {
            method: "GET",
            credentials: "include",
            headers: {
  ...
  return (
    <div>
      ...
      <UpperHeader
        ...
        subtitle={lecture ? lecture.description : ""}
      />
      <div className={styles.container}>
        {materialType.includes("course") && (
          <Link
            to={`/view-course-teacher/${idCourses}/${idLectures}/course`}
        ...
        {materialType.includes("auxiliar") && (
          <Link
            to={`/view-course-teacher/${idCourses}/${idLectures}/auxiliar`}
            className={styles.noDecoration}
          >
        ...
        {materialType.includes("video") && (
          <Link
            to={`/view-course-teacher/${idCourses}/${idLectures}/video`}
            className={styles.noDecoration}
          >
        ...
        {!hasError && <Buttons idLectures={idLectures} idCourses={idCourses} />}
      </div>
    </div>
  );
}
