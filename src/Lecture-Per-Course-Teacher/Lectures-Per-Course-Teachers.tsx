...
function Buttons({ idCourses }: { idCourses: string | undefined }) {
  ...
  const handlePOST = async () => {
    const response = await fetch(
      `http://localhost:8081/api/v1/lectures/create/${idCourses}`,
      {
        method: "POST",
        credentials: "include",
        headers: {
    ...
    <div>
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          <DialogContent>
            <DialogContentText>
              To create a new week, please enter the name, description and week
              number here.
            </DialogContentText>
            <TextField
              autoFocus
...
function LecturesPerCourseTeachers() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(
          `http://localhost:8081/api/v1/courses/${idCourses}`,
          {
            method: "GET",
            credentials: "include",
            headers: {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(
          `http://localhost:8081/api/v1/lectures/idCourses=${idCourses}`,
          {
            method: "GET",
            credentials: "include",
            headers: {
  ...
  return (
    <div>
      ...
      <div className={styles.container}>
        ...
        {!hasError && <Buttons idCourses={idCourses} />}
      </div>
    </div>
  );
}
